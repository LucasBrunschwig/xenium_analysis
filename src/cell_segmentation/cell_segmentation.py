...
from src.utils import get_human_breast_he_path
...
def filter_transcripts(transcripts, min_qv, x_range: list = Optional[None], y_range: list = Optional[None]):

    transcripts = transcripts[transcripts["qv"] >= min_qv]

    # Change cell_id of cell-free transcripts from -1 to 0
    neg_cell_row = transcripts["cell_id"] == -1
    transcripts.loc[neg_cell_row, "cell_id"] = 0

    # Output filtered transcripts to CS
    transcripts_filtered_path = '_'.join(
         ["X" + str(x_range[0]) + "-" + str(x_range[1]), "Y" + str(y_range[0]) + "-" + str(y_range[1]),
          "filtered_transcripts.csv"])
    transcripts.to_csv(transcripts_filtered_path, index=False, encoding='utf-8')

    return transcripts, transcripts_filtered_path


def run_cell_segmentation(method: str, params: dict, scale: int, expand_nuclear_area: int, transcripts_path_: Path):

    print("1: Filtering Transcripts")
    transcripts_ = pd.read_csv(transcripts_path_)
    transcripts_filtered, transcripts_filtered_path_ = filter_transcripts(transcripts_, min_qv=20)
    ...
    baysor_cli = f"baysor run -c {toml_file} {str(transcripts_filtered_path_)}"  # {str(output_prior_segmentation)}"
...
def map_transcripts_to_cell(segmentation_path: Path, method: str):
    pass
...
if __name__ == "__main__":
    ...
    map_transcripts_to_cell(results_path, method="cellposed")
