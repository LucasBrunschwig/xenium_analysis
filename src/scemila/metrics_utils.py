    ...
    def score_proba(self, y_test: np.ndarray, y_pred_proba: np.ndarray):
        ...
        for metric in self.metrics:
            if metric == "aucprc":
                ...
                results[metric] = self.class_accuracy(y_test, y_pred)
            else:
                raise ValueError(f"invalid metric {metric}")

        return results

    def class_accuracy(self, y_test: np.ndarray, y_pred):
        labels = np.unique(y_test)
        class_acc = {int(i): [] for i in labels}
        for y, pred in zip(y_test, y_pred):
            class_acc[int(y)].append(1 if int(y) == int(pred) else 0)
        for i in labels:
            class_acc[int(i)] = np.sum(class_acc[int(i)]) / len(class_acc[int(i)])
        return class_acc
